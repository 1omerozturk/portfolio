...
exports.getEducations = async (req, res) => {
  try {
    const educations = sortDocs(
      await getCollectionDocs("educations"),
      "createdAt",
      "desc",
    );
...
exports.getExperiences = async (req, res) => {
  try {
    const experiences = sortDocs(
      await getCollectionDocs("experiences"),
      "createdAt",
      "desc",
    );
...
exports.getProjects = async (req, res) => {
  try {
    const projects = sortDocs(
      await getCollectionDocs("projects"),
      "createdAt",
      "desc",
    );
...
exports.getBlogs = async (req, res) => {
  try {
    ...
    const allBlogs = sortDocs(
      await getCollectionDocs("blogs"),
      "createdAt",
      "desc",
    );
    const filtered = allBlogs.filter((blog) => {
      ...
      if (search) {
        const haystack =
          `${blog.title || ""} ${blog.content || ""} ${blog.tags?.join(" ") || ""}`.toLowerCase();
    ...
    const totalBlogs = await getCollectionDocs("blogs");
...
exports.getBlogById = async (req, res) => {
  try {
    ...
    const blog = await getCollectionDocs("blogs",
      id,
      { $inc: { views: 1 } },
      { new: true },
    );
